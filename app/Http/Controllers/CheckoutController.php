...
class CheckoutController extends Controller
{
    ...
    public function oneClickCheckoutData()
    {
        try {
            if (!Auth::check()) {
                return response()->json([
            ...
            $userId = Auth::id();
            $carts = OneClickOrder::with('product')->where('user_id', $userId)
            ->get()
              ->map(function ($cart) {
...
public function oneClickOrders(Request $request)
{
    if (!Auth::check()) {
        return response()->json([
            'status' => false,
    ...
    $user = Auth::user();
    $cartItems = OneClickOrder::with('product')->where('user_id', $user->id)->get();
