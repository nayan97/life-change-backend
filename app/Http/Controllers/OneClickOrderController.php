...
public function index()
{
    if (Auth::check()) {
        ...
        $cartItems = OneClickOrder::with('product')
            ->where('user_id', $logid)
            ->get()
            ->map(function ($cart) {
...
public function update(Request $request, $id)
{
    ...
    $cart = OneClickOrder::where('id', $id)->where('user_id', $user->id)->firstOrFail();
...
public function destroy($id)
    {
        try {
            $cart = OneClickOrder::findOrFail($id);
            $cart->delete();
